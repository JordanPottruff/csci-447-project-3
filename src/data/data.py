...
class Data:
    ...
    def convert_data(self):
        ...
        for obs in self.data:
            cls = obs[self.class_col]
            # If the current class has not been encountered, we assign then next available index to it.
            if cls not in class_to_index:
                class_to_index[cls] = next_index
                next_index += 1
            # We then store the class' index as class_index:
            class_index = class_to_index[cls]
            # Finally, we generate the numpy array for the output.


            converted_cols = []
            for attr_col in self.attr_cols:
                converted_cols.append(obs[attr_col])
